        ...
        <div class="row">
            <div class="col-md-3 border border-primary rounded bg-white" style="font-size: large">
                ...
                <div class="mb-2">
                    ...
                    @foreach($type as $t)
                        @if($loop->first)
                            <input type="checkbox" name="type" id="" value="free"> Free ({{$t}})
                            <br>
                        @endif
                        @if($loop->last)
                            <input type="checkbox" name="type" id="" value="paid"> Paid ({{$t}})
                        @endif
                    @endforeach
                ...
                <div class="mb-2">
                    @foreach($level as $l)
                        @if($loop->first)
                            <input type="checkbox" name="medium" id="" style="border:1px solid red;"> Beginner ({{$l}})
                            <br>
                        @endif
                        @if($loop->last)
                            <input type="checkbox" name="medium" id=""> Advanced ({{$l}})
                        @endif
                    @endforeach
                </div>
                ...
                @foreach($technologies->courses as $tutorial)
                    <div class="col-md-12 d-flex border-bottom selected bg-white">
                        <div class="col-md-1 p-3">
                            <form action="{{URL::to('tutorials')}}/{{$tutorial->id}}" method="POST">
                                @csrf
                            <button type="submit" class="btn btn-dark btn-lg" id="vote" data-course-id="{{$tutorial->id}}"><i class="fas fa-caret-up fa-2x"></i>
                                <span> {{$tutorial->votes}}</span>
                            </button>
                            </form>
                        </div>
